...
- (FMDatabase*)database {
#if 0 // --twinlife-- 2022-01-19: we must not re-open the SQLcipher database: we don't know the secret key anyway.
    if (![_db isOpen]) {
        if (!_db) {
...
// --twinlife-- 2023-09-21: helper method to check if current thread is doing some database access
- (FMDatabaseQueue *)currentSyncQueue {

    return (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
}
// --twinlife-- 2023-09-21

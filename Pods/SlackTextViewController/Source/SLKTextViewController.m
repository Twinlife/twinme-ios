...
{
    if (!_textInputbar) {
        _textInputbar = [[SLKTextInputbar alloc] initWithTextViewClass:self.textViewClass toolbarViewClass:self.toolbarViewClass sendButtonViewClass:self.sendButtonViewClass recordButtonViewClass:self.recordButtonViewClass];
...
{
    [self setTextInputbarHidden:hidden animated:NO height:0];
}

- (void)setTextInputbarHidden:(BOOL)hidden animated:(BOOL)animated height:(CGFloat)height
{
    ...
    void (^animations)() = ^void(){
        ...
        weakSelf.textInputbarHC.constant = hidden ? height : weakSelf.textInputbar.appropriateHeight;
...
- (void)moveViewTo:(CGFloat)pos shownPhotoScroll:(BOOL)shownPhotoScroll {

    _willShowPhotoScroll = shownPhotoScroll;
    ...
    if ((pos > 0) && !_willShowPhotoScroll) {
        return;
    }
    // Updates the height constraints' constants
    self.keyboardHC.constant = pos;
    self.scrollViewHC.constant = [self slk_appropriateScrollViewHeight];
...
- (void)slk_willShowOrHideKeyboard:(NSNotification *)notification
{
    ...
    if (_willShowPhotoScroll && (SLKKeyboardStatusWillHide == status) && [self.textView isFirstResponder]) {
        return;
    }
...
- (void)slk_didChangeTextViewPasteboard:(NSNotification *)notification
{
    ...
    if (notification.userInfo) {
        ...
        [self didPasteStringContent:notification.object];
    }
}
...
- (void)registerClassForToolbarView:(Class _Nullable)aClass
{
    if (aClass == nil) {
        return;
    }
    ...
    NSAssert([aClass isSubclassOfClass:[UIView class]], @"The registered class is invalid, it must be a subclass of UIView.");
    self.toolbarViewClass = aClass;
}

- (void)registerClassForSendButtonView:(Class _Nullable)aClass
{
    if (aClass == nil) {
        return;
    }
    ...
    NSAssert([aClass isSubclassOfClass:[UIView class]], @"The registered class is invalid, it must be a subclass of UIView.");
    self.sendButtonViewClass = aClass;
}

- (void)registerClassForRecordButtonView:(Class _Nullable)aClass
{
    if (aClass == nil) {
        return;
    }
    ...
    NSAssert([aClass isSubclassOfClass:[UIView class]], @"The registered class is invalid, it must be a subclass of UIView.");
    self.recordButtonViewClass = aClass;
}
...
- (void)textView:(SLKTextView *)textView showFormatMenu:(CGRect)targetRect {
    ...
    UIEditMenuConfiguration *editMenuConfiguration = [UIEditMenuConfiguration configurationWithIdentifier:nil sourcePoint:targetRect.origin];
    [editMenuInteraction presentEditMenuWithConfiguration:editMenuConfiguration];
}
...
- (nullable UIMenu *)editMenuInteraction:(UIEditMenuInteraction *)interaction menuForConfiguration:(UIEditMenuConfiguration *)configuration suggestedActions:(NSArray<UIMenuElement *> *)suggestedActions {
    ...
    return [UIMenu menuWithTitle:@"" children:@[boldAction, italicAction, strikeThroughAction]];
}
...
- (void)slk_registerNotifications
{
    ...
    [notificationCenter addObserver:self selector:@selector(slk_didChangeTextViewPasteboard:) name:SLKTextViewDidPasteStringNotification object:nil];
...
- (void)dealloc
{
    ...
    _toolbarViewClass = nil;
    _sendButtonViewClass = nil;
    _recordButtonViewClass = nil;

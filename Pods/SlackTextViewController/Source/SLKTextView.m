...
NSString * const SLKTextViewDidPasteStringNotification =            @"SLKTextViewDidPasteStringNotification";
...
- (void)paste:(id)sender
{
    ...
    if ([pastedItem isKindOfClass:[NSDictionary class]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:SLKTextViewDidPasteStringNotification object:pastedItem userInfo:nil];
...
- (void)slk_presentFormattingMenu:(id)sender
{
    ...
    if (@available(iOS 16.0, *)) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(textView:showFormatMenu:)]) {
            [self.delegate textView:self showFormatMenu:targetRect];
        }
    } else {
...
- (void)formatTextWithSymbol:(NSString *)symbol {

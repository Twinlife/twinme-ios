    ...
    if (self.restoreState == TLRestoreStateStarting && !self.restoreInProgress) {
        ...
        if (self.verifyMode && self.isVerifyBackupTerminated && self.restoreReport && ![self.restoreReport isRestoreUpToDate]) {
            ...
            if (![self.restoreReport.clickToCall isStatsUpToDate]) {
                [self.restoreItems addObject:[[UIRestoreItem alloc] initWithType:UIRestoreItemTypeSection text:TwinmeLocalizedString(@"premium_services_view_click_to_call_title", nil) icon:nil value:-1 color:nil]];
                
                if (self.restoreReport.clickToCall.added != 0) {
                    [self.restoreItems addObject:[[UIRestoreItem alloc] initWithType:UIRestoreItemTypeContent text:TwinmeLocalizedStringFromTable(@"backup_view_content_diff_added", @"LocalizableBackup", nil) icon:[UIImage imageNamed:@"AddExternalCall"] value:self.restoreReport.clickToCall.added color:Design.BLACK_COLOR]];
                }
                
                if (self.restoreReport.clickToCall.modified != 0) {
                    [self.restoreItems addObject:[[UIRestoreItem alloc] initWithType:UIRestoreItemTypeContent text:TwinmeLocalizedStringFromTable(@"backup_view_content_diff_updated", @"LocalizableBackup", nil) icon:[UIImage imageNamed:@"ActionEdit"] value:self.restoreReport.clickToCall.modified color:Design.BLACK_COLOR]];
                }
                
                if (self.restoreReport.clickToCall.deleted != 0) {
                    [self.restoreItems addObject:[[UIRestoreItem alloc] initWithType:UIRestoreItemTypeContent text:TwinmeLocalizedStringFromTable(@"backup_view_content_diff_deleted", @"LocalizableBackup", nil) icon:[UIImage imageNamed:@"DeleteItem"] value:self.restoreReport.clickToCall.deleted color:nil]];
                    [self.restoreItems addObject:[[UIRestoreItem alloc] initWithType:UIRestoreItemTypeInfo text:TwinmeLocalizedStringFromTable(@"restore_view_content_contact_deleted_message", @"LocalizableBackup", nil) icon:nil value:-1 color:nil]];
                }
            }

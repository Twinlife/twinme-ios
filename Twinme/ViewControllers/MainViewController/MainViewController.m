...
@interface MainViewController () <MainServiceDelegate, SplashScreenDelegate, SkredBoardViewControllerDelegate, UIGestureRecognizerDelegate, LockScreenDelegate, InAppPurchaseManagerDelegate, AcceptInvitationSubscriptionDelegate, AlertMessageViewDelegate, BottomSheetViewDelegate, MenuBackupViewDelegate>
...
- (void)unlockScreenSuccess {
    ...
    if (self.lockScreenViewController) {
        if ([self.twinmeApplication showWelcomeScreen] && !self.inPersonToCall) {
            WelcomeViewController *welcomeViewController = [self.storyboard instantiateViewControllerWithIdentifier:@"WelcomeViewController"];
            TwinmeNavigationController *welcomeNavigationController = [[TwinmeNavigationController alloc]initWithRootViewController:welcomeViewController];
            [self presentViewController:welcomeNavigationController animated:NO completion:^{
                [self.lockScreenViewController.view removeFromSuperview];
...
- (void)hideProgressIndicator {
    ...
    } else if (self.inPersonToCall && ![self.twinmeApplication showLockScreen]) {
        [self handleDeferredCall];
    }
}
...
- (void)handleDeferredCall {
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        if (subject) {
            ...
            BOOL isCallReceiver = [subject isKindOfClass:[TLCallReceiver class]];
            if (!isCallReceiver && subject.isGroup && ![self.twinmeApplication isSubscribedWithFeature:TLTwinmeApplicationFeatureGroupCall]) {
            ...
            } else if ([self hasSchedule:subject]) {

...
@interface PeerImageItemCell ()<UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AnnotationActionDelegate>
...
@property (nonatomic) BOOL imageWasNotAvailable;
...
//
// Implementation: PeerImageItemCell
//

#undef LOG_TAG
#define LOG_TAG @"PeerImageItemCell"
...
- (void)bindWithItem:(Item *)item conversationViewController:(ConversationViewController *)conversationViewController asyncManager:(AsyncManager *)asyncManager {
    ...
    CGFloat topMargin = [conversationViewController getTopMarginWithMask:peerImageItem.corners & ITEM_TOP_LEFT item:item];
    self.contentImageViewTopConstraint.constant = topMargin;
    self.replyViewTopConstraint.constant = topMargin;
    self.contentImageViewBottomConstraint.constant = -[conversationViewController getBottomMarginWithMask:peerImageItem.corners & ITEM_BOTTOM_LEFT item:item];
    ...
    // Trigger another image loader if we don't have the image
    // because it was not available in the past but now we have it.
    if (!self.imageLoader || (!self.imageLoader.image && self.imageLoader.isFinished && self.imageWasNotAvailable && [self.imageDescriptor isAvailable])) {
    ...
    if (image) {
        ...
        if ([UIImage isAnimatedImage:[[self.imageDescriptor getURL] absoluteString]] && [peerImageItem isAvailableItem]) {
    ...
    if (![peerImageItem isAvailableItem]) {
        self.gradientBottomView.hidden = NO;
        self.progressView.hidden = NO;
        self.progressLabel.hidden = NO;
        self.ephemeralView.hidden = YES;
        float progress = (self.imageDescriptor.end * 100) / self.imageDescriptor.length;
    ...
    if (peerImageItem.replyToDescriptor) {
        switch ([peerImageItem.replyToDescriptor getType]) {
            case TLDescriptorTypeObjectDescriptor: {
                self.replyView.hidden = NO;
                self.replyToImageContentView.hidden = YES;
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLObjectDescriptor *objectDescriptor = (TLObjectDescriptor *)peerImageItem.replyToDescriptor;
            ...
            case TLDescriptorTypeNamedFileDescriptor: {
                ...
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLNamedFileDescriptor *namedFileDescriptor = (TLNamedFileDescriptor *)peerImageItem.replyToDescriptor;
            ...
            case TLDescriptorTypeImageDescriptor: {
                ...
                if (!self.replyImageLoader) {
                    TLImageDescriptor *imageDescriptor = (TLImageDescriptor *)peerImageItem.replyToDescriptor;
            ...
            case TLDescriptorTypeVideoDescriptor: {
                ...
                if (!self.replyVideoLoader) {
                    TLVideoDescriptor *videoDescriptor = (TLVideoDescriptor *)peerImageItem.replyToDescriptor;
    ...
    int corners = peerImageItem.corners;

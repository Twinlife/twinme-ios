...
@interface MessageItemCell ()<UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AnnotationActionDelegate>
...
- (void)bindWithItem:(Item *)item conversationViewController:(ConversationViewController *)conversationViewController asyncManager:(AsyncManager *)asyncManager {
    ...
    self.contentLabelBottomConstraint.constant = -[conversationViewController getBottomMarginWithMask:messageItem.corners & ITEM_BOTTOM_RIGHT item:item];
    ...
    int countEmoji = [self countEmoji:messageItem.content];
    ...
    if (countEmoji == 0) {
        ...
        if (!messageItem.replyToDescriptor) {
    ...
    if (messageItem.replyToDescriptor) {
        switch ([messageItem.replyToDescriptor getType]) {
            case TLDescriptorTypeObjectDescriptor: {
                self.replyView.hidden = NO;
                self.replyToImageContentView.hidden = YES;
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLObjectDescriptor *objectDescriptor = (TLObjectDescriptor *)messageItem.replyToDescriptor;
            ...
            case TLDescriptorTypeNamedFileDescriptor: {
                ...
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLNamedFileDescriptor *namedFileDescriptor = (TLNamedFileDescriptor *)messageItem.replyToDescriptor;

...
@interface PeerLinkItemCell ()<UIGestureRecognizerDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AnnotationActionDelegate>
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *linkViewLeadingConstraint;
...
//
// Implementation: PeerLinkItemCell
//

#undef LOG_TAG
#define LOG_TAG @"PeerLinkItemCell"
...
- (void)bindWithItem:(Item *)item conversationViewController:(ConversationViewController *)conversationViewController asyncManager:(AsyncManager *)asyncManager {
    ...
    if (!self.linkLoader) {
        self.linkLoader = [[AsyncLinkLoader alloc] initWithItem:self.item objectDescriptor:peerLinkItem.objectDescriptor];
    ...
    if (peerLinkItem.replyToDescriptor) {
        switch ([peerLinkItem.replyToDescriptor getType]) {
            case TLDescriptorTypeObjectDescriptor: {
                self.replyView.hidden = NO;
                self.replyToImageContentView.hidden = YES;
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLObjectDescriptor *objectDescriptor = (TLObjectDescriptor *)peerLinkItem.replyToDescriptor;
            ...
            case TLDescriptorTypeNamedFileDescriptor: {
                ...
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLNamedFileDescriptor *namedFileDescriptor = (TLNamedFileDescriptor *)peerLinkItem.replyToDescriptor;
            ...
            case TLDescriptorTypeImageDescriptor: {
                ...
                if (!self.replyImageLoader) {
                    TLImageDescriptor *imageDescriptor = (TLImageDescriptor *)peerLinkItem.replyToDescriptor;
            ...
            case TLDescriptorTypeVideoDescriptor: {
                ...
                if (!self.replyVideoLoader) {
                    TLVideoDescriptor *videoDescriptor = (TLVideoDescriptor *)peerLinkItem.replyToDescriptor;
    ...
    int corners = peerLinkItem.corners;
    ...
    if (peerLinkItem.visibleAvatar && [conversationViewController displayPeerItemAvatar]) {
        ...
        if (![conversationViewController displayPeerItemAvatar]) {            
            if ([conversationViewController isSelectItemMode]) {
                self.contentLabelLeadingConstraint.constant = self.checkMarkViewLeadingConstraint.constant + self.checkMarkViewLeadingConstraint.constant + Design.AVATAR_CONVERSATION_LEADING;
            } else {
                self.contentLabelLeadingConstraint.constant = Design.AVATAR_CONVERSATION_LEADING;
            }
        }
    }

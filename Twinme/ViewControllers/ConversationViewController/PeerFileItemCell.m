...
@interface PeerFileItemCell ()<UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, AnnotationActionDelegate>
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *contentFileViewLeadingConstraint;
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *titleFileLabelViewHeightConstraint;
...
//
// Implementation: PeerFileItemCell
//

#undef LOG_TAG
#define LOG_TAG @"PeerFileItemCell"
...
- (void)awakeFromNib {
    ...
    self.contentFileViewLeadingConstraint.constant *= Design.WIDTH_RATIO;
    [self.contentFileView setBackgroundColor:Design.GREY_ITEM];
    ...
    self.titleFileLabelViewHeightConstraint.constant *= Design.HEIGHT_RATIO;
...
- (void)bindWithItem:(Item *)item conversationViewController:(ConversationViewController *)conversationViewController asyncManager:(AsyncManager *)asyncManager {
    ...
    CGFloat topMargin = [conversationViewController getTopMarginWithMask:peerFileItem.corners & ITEM_TOP_LEFT item:item];
    self.contentFileViewTopConstraint.constant = topMargin;
    self.replyViewTopConstraint.constant = topMargin;
    self.contentFileViewBottomConstraint.constant = -[conversationViewController getBottomMarginWithMask:peerFileItem.corners & ITEM_BOTTOM_LEFT item:item];
    ...
    if (![peerFileItem isAvailableItem]) {
        self.progressView.hidden = NO;
        self.progressLabel.hidden = NO;
        self.sizeFileLabel.hidden = YES;
        float progress = [Utils uploadProgressWithPosition:self.namedFileDescriptor.end length:self.namedFileDescriptor.length];
        self.progressView.progress = progress;
        self.progressLabel.text = [NSString stringWithFormat:@"%.0f %%", progress * 100.0];
    } else {
        self.progressView.hidden = YES;
        self.progressLabel.hidden = YES;
        self.sizeFileLabel.hidden = NO;
    }
    ...
    if (peerFileItem.replyToDescriptor) {
        switch ([peerFileItem.replyToDescriptor getType]) {
            case TLDescriptorTypeObjectDescriptor: {
                self.replyView.hidden = NO;
                self.replyToImageContentView.hidden = YES;
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLObjectDescriptor *objectDescriptor = (TLObjectDescriptor *)peerFileItem.replyToDescriptor;
            ...
            case TLDescriptorTypeNamedFileDescriptor: {
                ...
                self.replyViewTopConstraint.constant = topMargin;
                [self.replyLabel setPaddingWithTop:heightPadding left:widthPadding bottom:heightPadding right:widthPadding];
                TLNamedFileDescriptor *namedFileDescriptor = (TLNamedFileDescriptor *)peerFileItem.replyToDescriptor;
            ...
            case TLDescriptorTypeVideoDescriptor: {
                ...
                if (!self.replyVideoLoader) {
                    TLVideoDescriptor *videoDescriptor = (TLVideoDescriptor *)peerFileItem.replyToDescriptor;
    ...
    int corners = peerFileItem.corners;
...
- (void)onTouchUpInsideFile:(UITapGestureRecognizer *)tapGesture {
    ...
    if (![self.item isDeletedItem] && [self.fileActionDelegate respondsToSelector:@selector(openFileWithNamedFileDescriptor:)] && [self.item isAvailableItem]) {
...
- (void)drawRect:(CGRect)rect {
    ...
    CGFloat height = self.contentFileView.bounds.size.height - shadowOffset;

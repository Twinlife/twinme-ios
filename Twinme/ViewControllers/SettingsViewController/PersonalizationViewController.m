...
static const int SOUND_VIBRATION_SECTION = 4;
...
@interface PersonalizationViewController () <PersonalizationDelegate, SettingsActionDelegate, DisplayModeDelegate, MenuSelectColorDelegate, BottomSheetViewDelegate>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self isInformationPath:indexPath]) {
        ...
        if (indexPath.row == 1) {
            ...
            [cell bindWithTitle:TwinmeLocalizedString(@"personalization_view_system", nil) subTitle:nil icon:nil stateSwitch:self.twinmeApplication.displayMode == DisplayModeSystem tagSwitch:DISPLAY_MODE_SWITCH_TAG hiddenSwitch:NO disableSwitch:NO backgroundColor:Design.WHITE_COLOR hiddenSeparator:NO];
            return cell;
        }
    } else {
        SettingsItemCell *cell = [tableView dequeueReusableCellWithIdentifier:SETTINGS_CELL_IDENTIFIER];
...
- (BOOL)isInformationPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == DEFAULT_TAB_SECTION && indexPath.row == 0) {
        return YES;
    }

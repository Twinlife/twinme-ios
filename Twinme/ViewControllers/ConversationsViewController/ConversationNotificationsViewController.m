...
@interface ConversationNotificationsViewController () <SettingsActionDelegate, MenuSelectValueDelegate, EditContactCapabilitiesServiceDelegate>
...
- (void)initWithOriginator:(id<TLOriginator>)originator {
    ...
    self.discreetRelation = self.originator.identityCapabilities.hasDiscreet;
    
    if (!self.originator.identityCapabilities) {
        self.identityCapabilities = [[TLCapabilities alloc]init];
    } else {
        self.identityCapabilities = [[TLCapabilities alloc] initWithCapabilities:[self.originator.identityCapabilities attributeValue]];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == SECTION_DISCREET_RELATION) {
        ...
        if (indexPath.row > 1) {
            ...
            [cell bindWithTitle:title value:value icon:nil backgroundColor:Design.WHITE_COLOR];
...
- (void)saveCapabilities {
    ...
    if (![self.originator isKindOfClass:[TLGroup class]]) {
        TLContact *contact = (TLContact *)self.originator;
        [self.editCapabilitiesService updateIdentityWithContact:contact identityCapabilities:self.identityCapabilities];
    }
}
